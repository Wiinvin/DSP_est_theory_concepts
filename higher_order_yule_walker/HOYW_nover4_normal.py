...
def main():
    ...
    temp = lspec.high_order_yw(lynx, 2, 12, 100)
    ...
    for n in mdl_orders:
        ...
        l = dat_len / lag_division_factor
        m = dat_len / lag_division_factor

        print "\nOrder ", n, " HOYW System\n"
        hoyw_w = lspec.high_order_yw(lynx, n, m, l)
        print "For l, m values: ", l, m, " freq. estimates are: ", hoyw_w

        amp, phi = lspec.freqaphi(lynx, hoyw_w)
        print "Corresponding Amps are: ", amp
        print "Corresponding phases are: ", phi

        ## compute FFT for reference
        fft_w = np.fft.fft(lynx, axis=0)
        ...
        #
        plt.figure(1)
        plt.suptitle(plot_title + " N: " + str(dat_len) + " L: " + str(l) + " M: " + str(m))
        ...
        rel_mse[n] = lspec.calc_mse(reconst_sig, lynx)
        print "Relative Mean Squared Error is: ", rel_mse 

        aic_val[n] = lspec.aic(lynx, reconst_sig, n)
        print "AIC value for ", n, " parameters is: ", aic_val

        aicc_val[n] = lspec.aic(lynx, reconst_sig, n, correction=True)
        print "AICc value for ", n, " parameters is: ", aicc_val

        bic_val[n] = lspec.bic(lynx, reconst_sig, n)
